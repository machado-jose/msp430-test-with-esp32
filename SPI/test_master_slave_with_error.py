...
class Test_Template(unittest.TestCase):
	...
	def test_reading_registers_without_indicating_address(self):
		...
		# 1 - Objects Creation
		self.double_serial.repl("slave = Double_slave(13,12,14,15)",0.5)
		# 2 - Input Injection
		self.double_serial.repl("slave.enable_transaction("+str(expected_readings)+")",0.5)
		# 3 - Results gathering
	...
	def test_reading_registers_indicating_address(self):
		...
		self.double_serial.repl("slave = Double_slave(13,12,14,15)",0.5)
		# 2 - Input Injection
		self.double_serial.repl("slave.enable_transaction([0,1,2,3])",0.5)
		# 3 - Results gathering
		gotten_readings = self.dut_serial.repl("-a 14 3z", 1.2)[0]
		gotten_readings = gotten_readings.decode()
		gotten_readings = gotten_readings.split(',')
		gotten_readings = [int(i) for i in gotten_readings]
		gotten_written = self.double_serial.repl("slave.get_received_buffer()",0.5)[2]
	...
	def test_writing_registers_without_indicating_address(self):
		...
		self.double_serial.repl("slave = Double_slave(13,12,14,15)",0.5)
		# 2 - Input Injection 
		self.double_serial.repl("slave.enable_transaction([0,0,0])",0.5)
		self.dut_serial.repl("-s 1 2 3z", 1.2)
		# 3 - Results gathering
		gotten_values = self.double_serial.repl("slave.get_received_buffer()", 0.5)[2]
	...
	def test_writing_registers_indicating_address(self):
		...
		self.double_serial.repl("slave = Double_slave(13,12,14,15)",0.5)
		# 2 - Input Injection 
		self.double_serial.repl("slave.enable_transaction([0,0,0,0])",0.5)
